...
##fin=open( CFG.HAPI_HOME + 'capabilities.json','r')
##jset=json.loads(fin.read())
##fin.close()

jset = hp.get_hapiversion(CFG.HAPI_HOME)
...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_error(s,code,alt=400):
        ...
        try:
            myjson = hp.send_exception(msg)
            s.write(bytes(myjson,"utf-8"))
        except:
            pass
    ...
    def do_GET(s):
        ...
        if ( path=='hapi/capabilities' ):                
           ...
           lastModified = hp.get_lastModified(CFG.api_datatype, CFG.HAPI_HOME, id, timemin, timemax)
        ...
        if ( path=='hapi/capabilities' ):
            ...
            mystr = hp.do_write_info(id, parameters, CFG.HAPI_HOME, None )
            ...
            if check_error > 0:
                ...
                if query.__contains__('include'):
                    if query['include'][0]=='header':
                        hp.do_write_info( s, id, parameters, CFG.HAPI_HOME, '#' )
                ...
                #    CFG.floc (site-specific required elements from *_config.py)
                (status,data)=fetchdata(CFG.hapi_handler,
            ...
            mystr = hp.print_hapi_intropage(USE_CASE, CFG.HAPI_HOME)
            s.wfile.write(bytes(mystr,"utf-8"))

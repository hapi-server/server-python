...
def sm_parcel_keys(flagstring):
  # for allowed keys, places them as indices=, imf=, swi=

  imfkeys=["bgse","bgsm","vgse","vgsm"]
  swikeys=["pdyn","epsilon","newell","clockgse","clockgsm","density"]
  ...
  myflags = re.sub(',$','',myflags)
  if globaldebug: print("debug: flags are ",myflags)

  return(myflags)
      
## NEXT UP--- FIX THIS CHECK VIS A VIS THE ACTUAL PARAMETER FLAGS NEEDED
## VIA JSON AND MATRIX, OY
def sm_keycheck_indices(flagstring):
  ...
  #        
  print("Debug, flagstring is: ",flagstring)
...
def sm_GetUrl(fetchurl,fetch='list'):
  ...
  if globaldebug: print("debug: url trying ",fetch,"is",fetchurl)
...
def supermag_getinventory(logon,start,extent):
...
def supermag_getindices(logon,start,extent,flagstring='',**kwargs):
  # One of the core 3 functions
  ...
  indices = sm_parcel_keys(flagstring)
  urlstr += indices
...
def supermag_getdata(logon,start,extent,flagstring,stationlist,**kwargs):
  # One of the core 3 functions
  ...
  if type(stationlist) is not list:
    # single item
    (status,data)=sm_supermag_getdata_single(logon,start,extent,flagstring,stationlist,**kwargs)
  else:
    # multiple stations, append into larger dataframe
    frames=[]
    status=0
    for station in stationlist:
      (tstatus,tdata)=sm_supermag_getdata_single(logon,start,extent,flagstring,station,**kwargs)
      frames.append(tdata)
      status += tstatus
...
def sm_supermag_getdata_single(logon,start,extent,flagstring,station,**kwargs):

  typecode = kwargs.get('FORMAT','json').lower()
  urlstr = sm_coreurl('data-api.php',logon,start,extent)
  #print("debug: flagstring pre-check is ",flagstring)
  indices = sm_keycheck_data(flagstring)
  #print("debug: flagstring post-check is ",flagstring)
  urlstr += indices
  urlstr += '&' + flagstring
...
def sm_microtest(choice,userid):
  ...
  if choice == 1 or choice == 4:
    (status,stations) = supermag_getinventory(userid,start,3600)
    print(status)
    print(stations)

  if choice == 2 or choice == 4:
    (status,data) = supermag_getdata(userid,start,3600,'all,delta=start,baseline=yearly','HBK')
    print(status)
    print(data)
    print(data.keys())
  ...
  if choice == 3 or choice == 4:
    (status,idxdata) = supermag_getindices(userid,start,3600,'swiall,density,darkall,regall,smes')
    #print(status)
    #print(idxdata)
    idxdata.keys()
    tval=idxdata.tval
...
def supermag_testing(userid):
  ...
  (status,mydata1a) = supermag_getdata(userid,start,3600,'','HBK')
  ...
  (status,mydata1a) = supermag_getdata(userid,start,3600,'all','HBK')
  ...
  (status,mydata1b) = supermag_getdata(userid,start,3600,'','HBK',FORMAT='list')
  ...
  (status,mydata1b) = supermag_getdata(userid,start,3600,'all','HBK',FORMAT='list')
  ...
  ####################
  # INDICES fetches
  (status,idxdata) = supermag_getindices(userid,start,3600)
  idxdata  # empty!

  (status,idxdata) = supermag_getindices(userid,start,3600,'all,swiall,imfall')
  ...
  (status,mydata2c) = supermag_getindices(userid,start,3600,'all,swiall,imfall',FORMAT='list')
